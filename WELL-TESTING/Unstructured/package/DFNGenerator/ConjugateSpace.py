...
def check_proximity(frac1, frac2, threshold):
    ...
    num_segments = int(max(length_frac1, length_frac2) / (threshold))
    if num_segments==0:
        x1_long = long_frac[0]
        y1_long = long_frac[1]
        x2_long= long_frac[2]
...
class GenerateConjugateDFNWithSpace:
    def __init__(self, Ix,Iy,LminX,LminY,proximity_threshold ,outputDir, savePic=False):
        # -------------------------General Information----------------------------
        ...
        DoE = np.array(np.meshgrid(targetFracIntenDoE, targetFracIntenDoEJ, LengthMinDoE, LengthMinDoEJ,proximity_threshold)).T.reshape(-1, 5)
        ...
        for DoELoop in range(sizeDoE):
            ...
            proximity_threshold=DoE[DoELoop][4]
            ...
            self.conjugate_fractures = []  # Each element is a tuple: (x1, y1, x2, y2)
            ...
            for multRealisations in range(maxNumberRealisationsperCase):
                ...
                with open(filename, 'w') as fileID:
                    ...
                    while p32 <= targetFracIntensity and numIter < maxNumIter and warningCheck < 50000:
                        ...
                        while retry_count < max_retries:
                            ...
                            xCoordFrac2_new = np.round(xCoordFrac1 + fracLength[-1] * np.cos(np.radians(30)),
                                                   decimals=4)
                            yCoordFrac2_new = np.round(yCoordFrac1 + fracLength[-1] * np.sin(np.radians(30)),
                                                   decimals=4)
                            if xCoordFrac2_new < maxXcoord and yCoordFrac2_new < maxYcoord:
                                ...
                                superimpose_condition = any(
                                    yCoordFrac1 == existing_frac[1] and (
                                            ...
                                            (xCoordFrac2_new >= existing_frac[0] and xCoordFrac2 <= existing_frac[2])
                                ...
                                if not superimpose_condition:
                                    proximity_condition = any(
                                        check_proximity((xCoordFrac1, yCoordFrac1, xCoordFrac2_new, yCoordFrac2_new), existing_frac,
                                                        proximity_threshold)
                                        for existing_frac in self.conjugate_fractures
                                    )

                                if not (superimpose_condition or proximity_condition):
                                        break
                                  # Exit the retry loop if conditions are met
                        ...
                        if retry_count < max_retries:
                            ...
                            fracLengthCorr = xCoordFrac2_new - xCoordFrac1
                            ...
                            fileID.write(f"{xCoordFrac1} {yCoordFrac1} {xCoordFrac2_new} {yCoordFrac2_new}\n")
                            self.horizontal_fractures.append((xCoordFrac1, yCoordFrac1, xCoordFrac2, yCoordFrac2))
                            self.conjugate_fractures.append((xCoordFrac1, yCoordFrac1, xCoordFrac2_new, yCoordFrac2_new))
                ...
                if savePic:
                    ...
                    for frac in self.conjugate_fractures:
